...
describe('BankUpdateForm component', async () => {
    const routeToUpdateBankOne = RoutePaths.BankUpdate.replace(':id','1');
    it('calls the query method on Apollo Client', async () => {
        const wrapper = mount(
            customWrapper(<BankUpdateForm />, [], routeToUpdateBankOne, RoutePaths.BankUpdate)
        );
    ...
    it('renders without crashing', async () => {
        const wrapper = mount(
            customWrapper(<BankUpdateForm />, [...bankMock, ...updateBankMock], routeToUpdateBankOne, RoutePaths.BankUpdate)
        );
        expect(wrapper.find(Spinner).length).toEqual(1);
