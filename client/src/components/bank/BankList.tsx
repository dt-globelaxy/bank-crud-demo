...
class BankList extends React.Component<RouteComponentProps> {
    ...
    handleCreateRedirect = () => { this.props.history.push(RoutePaths.BankCreate); }
    render() {
        return(<>
            <BankDelete openDialog={this.state.openDialog} item={this.state.item} closeDialog={this.handleDialogClose} />
            ...
            <Query query={GET_BANKS}>
                ...
                return (
                    <EnhancedTable title="Banks"  keyField="id" order={OrderType.asc} orderBy={'name'} isLoading={loading} data={data.getBanks}
                        columns={[
                        ...
                        { label: 'Actions', field: 'actions', disablePadding: true, action: true, actions: [ 
                            { label: 'Edit', actionIcon: <EditIcon/>, onClick: (item) => this.props.history.push(RoutePaths.BankUpdate.replace(':id',item.id)) },
