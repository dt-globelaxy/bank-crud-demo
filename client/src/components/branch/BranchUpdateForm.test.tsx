...
describe('BranchUpdateForm component', async () => {
    const routeToUpdateBranchOne = RoutePaths.BranchUpdate.replace(':id','1');
    it('calls the query method on Apollo Client', async () => {
        const wrapper = mount(
            customWrapper(<BranchUpdateForm />, [...branchMock], routeToUpdateBranchOne, RoutePaths.BranchUpdate)
        );
    ...
    it('renders without crashing', async () => {
        const wrapper = mount(
            customWrapper(<BranchUpdateForm />, 
                [...getBanksMock,...branchMock, ...updateBranchMock], 
                routeToUpdateBranchOne, RoutePaths.BranchUpdate)
        );
        expect(wrapper.find(Spinner).length).toEqual(1);
