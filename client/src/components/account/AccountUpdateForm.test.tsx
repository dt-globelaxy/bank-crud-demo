...
describe('AccountUpdateForm component', async () => {
    const routeToUpdateAccountOne = RoutePaths.AccountUpdate.replace(':id','1');
    it('calls the query method on Apollo Client', async () => {
        const wrapper = mount(
            customWrapper(<AccountUpdateForm />, accountMock, routeToUpdateAccountOne, RoutePaths.AccountUpdate)
        );
    ...
    it('renders without crashing', async () => {
        const wrapper = mount(
            customWrapper(<AccountUpdateForm  />, 
                [...accountMock, ...updateAccountMock, ...getBanksMock, ...getBranchesMock, ...accountMock,...getAccountsMock], 
                routeToUpdateAccountOne, RoutePaths.AccountUpdate)
        );
        expect(wrapper.find(Spinner).length).toEqual(1);
