...
interface QueryGetBanksResponse {
  getBanks: IBank[];
}

export const BankPage: React.FC = () => {
  const { loading, error, data } = useQuery<QueryGetBanksResponse>(GET_BANKS);

  if (loading || !data || !data.getBanks) return <Spinner />;
          ...
          {({ values }: { values: IAccountFormModel }) => {
            ...
            return (
              <Query
                ...
                variables={{ bankId: values.bankId }}
              >
                {({ data, error, loading }: QueryResult<any>) => {
                  if (error) {
                    return <Error error={error} />;
                  }
                  if (loading || !data.getBankBranches) {
                    return <Spinner />;
                  }
                  const branchOptions = data.getBankBranches.map(
                    (item: IBranch) => ({
                      value: item.id,
                      label: item.name,
                    })
                  ...
                  return (
                    <Field
                      ...
                      parse={(val) => val && val.value}
                      format={(val) =>
                        find(branchOptions, (o) => o.value === val)
                      }
                    >
                      {({ input, meta }) => (
                        <CustomSelect
