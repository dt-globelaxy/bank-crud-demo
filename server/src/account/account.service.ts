...
import { Repository, FindManyOptions, OrderByCondition } from 'typeorm';
...
export class AccountService {
    ...
    async findOneById(id: number): Promise<Account> {
        return await this.accountRepository.findOne(id, {relations: ['bank', 'branch']});
    }

    async findAllPaged(page: number, perPage: number, order: OrderType, orderBy: string,
                       fromNumber?: number | undefined, toNumber?: number | undefined): Promise<Paged<Account>> {
        const take = perPage;
        ...
        const orderByProp = orderBy.indexOf('.') > -1 ? orderBy : 'account.' + orderBy;
        const [result, total] = await query
            .orderBy({
                [orderByProp]: order.toUpperCase(),
            } as OrderByCondition)
            .take(take)
            .skip(skip).getManyAndCount();
